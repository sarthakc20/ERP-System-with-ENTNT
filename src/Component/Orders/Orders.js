  ...
  return (
    ...
    <div className="orders-management">
      ...
      <div className="view-toggle">
        <button className="toggle-btn" onClick={() => setView("table")}>Table View</button>
        <button className="toggle-btn" onClick={() => setView("calendar")}>Calendar View</button>
      </div>
      {view === "table" ? (
        <table>
          <thead>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id}>
                ...
                <td>
                  <button className="btn" onClick={() => deleteOrder(order.orderId)}>
                    Delete
                  </button>
                  <button className="MediaScreen" onClick={() => deleteOrder(order.orderId)}>
                  <MdDelete />
                  </button>
                </td>
              </tr>
          ...
          <Calendar
            ...
            tileContent={({ date, view }) => {
              if (view === "month") {
                ...
                return orderCount > 0 ? <p style={{ color: 'red' }}>{orderCount} Orders</p> : null;
              }
            }}
          />
